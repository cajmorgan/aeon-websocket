...
class Socket extends EventEmitter {
  ...
  send(message, opcode = 0x1) {
    ...
    if (messageLength < 180) {
      ...
      // let messageStream = [];
      // for (let i = 0; i < 150; i++)
      //   messageStream.push(messageBank.shift());

      // console.log(message)
      // const fullBufferOne = Buffer.from([0x1, 126, 0, 150, ...messageStream]);
      // this.socket.write(fullBufferOne);

      // messageStream = [];
      // for (let i = 0; i < 10; i++)
      //   messageStream.push(messageBank.shift());
      // const fullBufferTwo = Buffer.from([0x80, 10, ...messageStream]);
      // this.socket.write(fullBufferTwo);
